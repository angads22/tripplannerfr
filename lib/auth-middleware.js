...
// An optional "early bird" code that also lets people sign up, on by default
// so friends can get in right away. The admin can flip it off or change it.
function earlyBird() {
  const s = db.getSettings();
  return {
    enabled: s.earlyBirdEnabled !== undefined ? !!s.earlyBirdEnabled : true,
    code: s.earlyBirdCode || "potato 21",
  };
}

// Does this code let someone register? Matches the invite code, or the
// early-bird code when it's enabled (both case-insensitive, trimmed).
function codeAccepted(code) {
  const given = String(code || "").trim().toLowerCase();
  if (!given) return false;
  if (given === String(inviteCode()).trim().toLowerCase()) return true;
  const eb = earlyBird();
  return eb.enabled && given === String(eb.code).trim().toLowerCase();
}
